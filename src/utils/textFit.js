...
export function textFit(els, html, options) {
	if (!options) options = {};
	...
	for (var i = 0; i < els.length; i++) {
		processItem(els[i], html, settings);
	}
}
 ...
 */
function processItem(el, innerHtml, settings) {
	console.log("processing item", el, "Inner HTML: ", el.innerHTML);
	...
	// Get element data.
	originalHTML = innerHtml; // el.innerHTML;
	...
	console.log("Span about to be added: ", originalHTML, originalHTML.indexOf("textFitted") === -1);
	...
	if (originalHTML.indexOf("textFitted") === -1) {
		...
		innerSpan.style["display"] = "inline-block"; // "inline-flex";
	...
	var size = low;
	while (low <= high) {
		console.log("Binary Search Calculation: ", low, high);
		mid = (high + low) >> 1;
	...
	console.log("Size px", innerSpan, size + "px");
